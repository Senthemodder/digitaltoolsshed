...
export function renderUnitConverterBody(tool) {
  ...
  return `
    <div class="article-container" style="max-width: 950px; margin: 0 auto; padding: 1.5rem 1rem;">
      ...
      <!-- 5 Fatal Historical Metrology Traps & Calculation Pitfalls -->
      <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 1.5rem; margin-bottom: 2rem;">
        <h3 style="font-family: var(--serif); font-size: 1.25rem; margin-top: 0; margin-bottom: 0.75rem; color: var(--fg);">⚠️ 5 Fatal Historical Metrology Traps & Calculation Pitfalls</h3>
        ...
        <div style="display: grid; gap: 1rem;">
          <div class="trap-card" style="border-left: 4px solid #ef4444; background: var(--surface-alt); border: 1px solid var(--border); border-left: 4px solid #ef4444; border-radius: 6px; padding: 1rem 1.25rem;">
            <strong style="color: var(--fg); font-size: 0.95rem; display: block; margin-bottom: 0.25rem;">1. Provincial Metrological Drift & Epochal Inflation</strong>
          ...
          <div class="trap-card" style="border-left: 4px solid #f59e0b; background: var(--surface-alt); border: 1px solid var(--border); border-left: 4px solid #f59e0b; border-radius: 6px; padding: 1rem 1.25rem;">
            <strong style="color: var(--fg); font-size: 0.95rem; display: block; margin-bottom: 0.25rem;">2. Dry vs. Liquid Vessel Discrepancies & Struck vs. Heaped Measures</strong>
            <p style="margin: 0.35rem 0 0; font-size: 0.88rem; color: var(--text-muted); line-height: 1.5;">
          ...
          <div class="trap-card" style="border-left: 4px solid #10b981; background: var(--surface-alt); border: 1px solid var(--border); border-left: 4px solid #10b981; border-radius: 6px; padding: 1rem 1.25rem;">
            <strong style="color: var(--fg); font-size: 0.95rem; display: block; margin-bottom: 0.25rem;">3. Commodity Packing Density & Mass-Volume Confusion</strong>
            <p style="margin: 0.35rem 0 0; font-size: 0.88rem; color: var(--text-muted); line-height: 1.5;">
          ...
          <div class="trap-card" style="border-left: 4px solid #3b82f6; background: var(--surface-alt); border: 1px solid var(--border); border-left: 4px solid #3b82f6; border-radius: 6px; padding: 1rem 1.25rem;">
            <strong style="color: var(--fg); font-size: 0.95rem; display: block; margin-bottom: 0.25rem;">4. Non-Decimal Fractional Hierarchies & Compounding Rounding Error</strong>
            <p style="margin: 0.35rem 0 0; font-size: 0.88rem; color: var(--text-muted); line-height: 1.5;">
          ...
          <div class="trap-card" style="border-left: 4px solid #8b5cf6; background: var(--surface-alt); border: 1px solid var(--border); border-left: 4px solid #8b5cf6; border-radius: 6px; padding: 1rem 1.25rem;">
            <strong style="color: var(--fg); font-size: 0.95rem; display: block; margin-bottom: 0.25rem;">5. Artifact Tool Wear vs. De Jure Statutory Standards</strong>
            <p style="margin: 0.35rem 0 0; font-size: 0.88rem; color: var(--text-muted); line-height: 1.5;">
...
export function buildHistoryUnitsTools({ DIST, DOMAIN, renderPage, writeFileSync, join, ensureDir }) {
  ...
  for (const tool of HISTORY_UNITS_TOOLS) {
    const existingFaqs = tool.faq || [];
    const standardFaqs = [
      {
        q: `How accurate is this ${tool.name} converter compared to primary archaeological sources?`,
        a: `This calculator is mathematically calibrated against consensus metrological research and surviving primary artifacts (such as museum bronze standards, preserved cubit rods, and imperial statutes). It achieves sub-millimeter and micro-liter precision relative to the canonical ${tool.primaryUnit} standard of ${tool.era}.`
      },
      {
        q: `Why might ancient texts report different values for ${tool.primaryUnit}?`,
        a: `Ancient and medieval measurement systems predated state-enforced decimal standards. Commercial port cities, military garrisons, and agricultural provinces frequently adapted units to local trade traditions or saw definitions drift over centuries. Our converter provides the scholarly consensus benchmark while documenting historical variances in the contextual notes.`
      },
      {
        q: `Is this historical unit conversion processed locally and privately?`,
        a: `Yes. All conversions between ${tool.primaryUnit}, modern Metric (${tool.metricSymbol}), modern Imperial (${tool.imperialSymbol}), and classical subdivisions execute 100% client-side in your browser memory. Zero calculation inputs are transmitted to external servers, ensuring instant response and complete privacy.`
      }
    ];

    const allFaqs = [...existingFaqs];
    for (const sf of standardFaqs) {
      if (!allFaqs.some(f => f.q === sf.q)) {
        allFaqs.push(sf);
      }
    }
    tool.faq = allFaqs;
